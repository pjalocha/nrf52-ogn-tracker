...
{ public:
  ...
  public:
   LoRaWANnode() { Reset(); TxOptLen=0; TxBattLevel=0xFF; RxRSSI=0; RxSNR=0; }
...
#ifdef WITH_NRF52
  int WriteToNVS(const char *Name="/lorawan.dat")
  { setCRC();
#ifdef WITH_INTERNAL_FS
    InternalFS.remove(Name);
    Adafruit_LittleFS_Namespace::File WANfile = InternalFS.open(Name, Adafruit_LittleFS_Namespace::FILE_O_WRITE);
    if(!WANfile) return -1;
    int Written = WANfile.write((const uint8_t *)this, SaveBytes);
    WANfile.close();
    return Written==SaveBytes ? Written : -2;
#else
    return LogFS_writeFile(Name, this, SaveBytes);
#endif
  }

  int ReadFromNVS(const char *Name="/lorawan.dat")
  {
#ifdef WITH_INTERNAL_FS
    Adafruit_LittleFS_Namespace::File WANfile = InternalFS.open(Name, Adafruit_LittleFS_Namespace::FILE_O_READ);
    if(!WANfile) return -1;
    int Read = WANfile.read((uint8_t *)this, SaveBytes);
    WANfile.close();
    if(Read!=SaveBytes) return -2;
#else
    int Read=LogFS_readFile(Name, this, SaveBytes);
